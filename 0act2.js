/* act2.js — ΠΡΑΞΗ 2 (σταθερό timing, σωστός GHOST, HTML σε bubbles)
   ...
   - Τέλος: κρύβει δείκτη, κλείνει κουρτίνα (μένει κλειστή), εμφανίζει πύλη Φουαγιέ και dispatch act2-ended ΜΟΝΟ μία φορά.
*/
(function(){
  'use strict';
  ...
  let running=false, finished=false, dispatched=false;
  ...
  const Aghost_m = 5.0;
  const HOOK_NUDGE_PX = 0; // μικροδιόρθωση αν ποτέ χρειαστεί
  ...
  function startGhost(){
    ...
    const loop=()=>{
      ...
      // Κέντρο ghost = hookX_ghost (translate(-50%,0))
  ...
  async function playAct2(){
    ...
    try{
      ...
      // Dispatch ΜΙΑ φορά και εμφάνιση πύλης φουαγιέ
      if(!dispatched){
        try { document.dispatchEvent(new Event('act2-ended')); console.log('[Act2] dispatched act2-ended'); }
        catch(e){ console.error('act2-ended dispatch failed', e); }
        dispatched = true;
      }
      showFoyerGate();
      finished=true;
